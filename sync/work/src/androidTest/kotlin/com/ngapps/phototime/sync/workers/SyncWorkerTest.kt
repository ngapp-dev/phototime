...
class SyncWorkerTest {
    ...
    fun setup() {
        ...
        // NOTE: Initialize WorkManager for instrumentation tests.
    ...
    fun testSyncWork() {
        ...
        // NOTE: Enqueue and wait for result.
        workManager.enqueue(request).result.get()

        // NOTE: Get WorkInfo and outputData
        val preRunWorkInfo = workManager.getWorkInfoById(request.id).get()

        // NOTE: Assert
        assertEquals(WorkInfo.State.ENQUEUED, preRunWorkInfo.state)

        // NOTE: Tells the testing framework that the constraints have been met
