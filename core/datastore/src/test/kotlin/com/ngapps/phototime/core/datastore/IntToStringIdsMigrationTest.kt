...
class IntToStringIdsMigrationTest {
    ...
    fun IntToStringIdsMigration_should_migrate_topic_ids() = runTest {
        // NOTE: Set up existing preferences with topic int ids
        val preMigrationUserPreferences = userPreferences {
            deprecatedIntFollowedTopicIds.addAll(listOf(1, 2, 3))
        }
        // NOTE: Assert that there are no string topic ids yet
        assertEquals(
            emptyList<String>(),
        ...
        // NOTE: Assert the deprecated int topic ids have been migrated to the string topic ids
        assertEquals(
            userPreferences {
    ...
    fun IntToStringIdsMigration_should_migrate_author_ids() = runTest {
        // NOTE: Set up existing preferences with author int ids
        val preMigrationUserPreferences = userPreferences {
            deprecatedIntFollowedAuthorIds.addAll(listOf(4, 5, 6))
        }
        // NOTE: Assert that there are no string author ids yet
        assertEquals(
            emptyList<String>(),
        ...
        // NOTE: Assert the deprecated int author ids have been migrated to the string author ids
        assertEquals(
            userPreferences {
