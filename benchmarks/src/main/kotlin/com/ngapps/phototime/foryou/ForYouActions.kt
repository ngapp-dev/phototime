...
import com.ngapps.phototime.waitAndFindObject
...
fun MacrobenchmarkScope.forYouSelectTopics(recheckTopicsIfChecked: Boolean = false) {
    ...
    while (visited < 3) {
        if (topics.childCount == 0) {
            Assert.fail("No topics found, can't generate profile for ForYou page.")
        }
...
fun MacrobenchmarkScope.setAppTheme(isDark: Boolean) {
    ...
    waitForObjectOnTopAppBar(By.text("Shoot in time"))
}
